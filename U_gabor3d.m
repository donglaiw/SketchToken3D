%The following code creates a spatiotemporal 3d Gabor filter that prefers a given speed and direction of motion
 ...
 function result = U_gabor3d(v,theta,xi,opt,bandwidth)
 ...
 slratio = (1/pi) * sqrt( (log(2)/2) ) * ( (2^bandwidth + 1) / (2^bandwidth - 1) ); sigma = slratio * lambda;
 ...
 theta = pi - theta;
 %rotation matrix 
 xt = x * cos(theta) + y * sin(theta); yt = -x * sin(theta) + y * cos(theta); tt = t;
 %Fix vf depending on whether the kernel has a stationary envelope or moving envelope 
 if(opt == 0) vf = 0; else vf = v; end
 

 %precompute few constants
 %components of the cosine factor 
 xp = xt + v*t; f1 = 2*pi/lambda; b = 1 / (2*sigma*sigma); tau2 = 1/(2*tau*tau); gamma2 = gamma*gamma; a = gamma / (2*sigma*sigma*pi);
 %normalization for spatial gaussian 
 t1 = 1/(sqrt(2*pi)*tau);
 
 %normalization for temporal gaussian
 ...
 ppos = find(result > 0);
 %pointer list to indices of elements of result which are positive 
 pneg = find(result < 0);
 %pointer list to indices of elements of result which are negative 
 pos = sum(result(ppos));
 % sum of the positive elements of result 
 neg = abs(sum(result(pneg)));
 % abs value of sum of the negative elements of result 
 meansum = (pos+neg)/2; 
 if (meansum > 0) 
 	pos = pos / meansum;
 % normalization coefficient for negative values of result 
 	neg = neg / meansum;
 % normalization coefficient for psoitive values of result 
 end 
 result(pneg) = pos*result(pneg); result(ppos) = neg*result(ppos);
 ...
 %and the minimum value of the kernel (say, maxker and minker, respectively) and add them as options in imagesc function in
 %order to see the change of intensity of the excitatory and inhibitory lobes of the kernel over different frames.
 %For example if the kernel is of size 16 * 16 * 16, one could use imagesc(result(:,:,9),[minker,maxker]); to view 9th
 %frame.
